# Slice 1 Schema Proposal

Status: approved direction applied, not migrated into a database yet.

This schema implements the `docs/design.md` slice 1 foundation: batch-level upload tracking, staging, normalized facility-centered tables, data quality issue storage, bulletin run metadata, and placeholders for the report-ready metrics layer.

The proposal now reflects the requested adjustments:

- Uploads are grouped by `upload_batches`.
- All facility-scoped tables include `tenant_id` and `country_code`.
- `facility_id` remains the local natural identifier, but the database key is tenant/country/facility.
- Binary fields are stored as `BOOLEAN`.
- Ambiguous yes/no/partial fields are stored as enums rather than free text.
- Percentages are normalized as fractions from `0.0` to `1.0`, with light SQL bounds checks.
- Governance, workforce, and operations records are current snapshots effective across the clinical period unless dates are supplied.
- Cross-field logical inconsistencies are intentionally not hard SQL constraints; slice 2 data quality rules will capture and flag bad data without rejecting it.

No migration has been applied yet.

## Design Choices

- `upload_batches` represents one user refresh or bulletin input bundle.
- `uploads` represents one file inside a batch and links to `upload_batches.id`.
- Static files use nullable `effective_start_month` and `effective_end_month`; null means "current snapshot applies to the batch/reporting context."
- Percentage columns use `NUMERIC(6, 5)` with `value IS NULL OR value BETWEEN 0 AND 1` checks.
- Simple non-negative checks remain on count/capacity fields because negative counts are malformed values, not meaningful health anomalies.
- Cross-field checks such as deaths greater than live births, stillbirths greater than deliveries, preterm births greater than live births, and functional equipment greater than total equipment belong in `data_quality_issues`.
...
CREATE INDEX ix_uploads_batch_id
    ON uploads (batch_id);

CREATE INDEX ix_uploads_dataset_kind_created_at
    ON uploads (dataset_kind, created_at DESC);
...
CREATE INDEX ix_staging_rows_batch_id
    ON staging_rows (batch_id);

CREATE INDEX ix_staging_rows_upload_id
    ON staging_rows (upload_id);

CREATE INDEX ix_staging_rows_dataset_kind
    ON staging_rows (dataset_kind);
...
CREATE INDEX ix_data_quality_issues_severity
